...
func FetchObject(c *Client, className, objectId, include string) (*Object, error) {
	if className == "" || objectId == "" {
		return nil, ErrNoObjectIdOrClassName
	}
	r, err := c.getObject(className, objectId, include)
	if err != nil {
		return nil, err
	}
	return r.Decode()
...
func DeleteObject(c *Client, className, objectId string) error {
	...
	_, err := c.deleteObject(className, objectId)
	return err
}
...
func (o *Object) Save(c *Client, className string, fetchOnSave bool) error {
	r, err := c.createObject(className, o.Encode())
	if !fetchOnSave || err != nil {
		return err
	}
	r, err = c.getObjectDirectly(r.Location)
	if err != nil {
		return err
	}
	o1, err := r.Decode()
...
func (o *Object) Update(c *Client, className string) error {
	if o.ObjectId() == "" || className == "" {
		return ErrNoObjectIdOrClassName
	}
	_, err := c.updateObject(className, o.ObjectId(), o.Encode())
	return err
}

func (o *Object) Delete(c *Client, className string) error {
	if o.ObjectId() == "" || className == "" {
		return ErrNoObjectIdOrClassName
	}
	_, err := c.deleteObject(className, o.ObjectId())
	return err
}
